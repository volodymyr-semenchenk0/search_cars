...
from app.repositories.car_repository import CarRepository  # Для створення авто
...
class OfferRepository:
    ...
    def create_full_offer_with_details(data: Dict[str, Any]) -> Optional[int]:
        ...
        try:
            ...
            if not car_id:
                # Помилка вже залогована в create_car_with_powertrain
                conn.rollback()
                return None
    ...
    def delete_cascade(offer_id: int) -> bool:
        ...
        try:
            ...
            car_id_to_delete = result[0] if isinstance(result, tuple) else result.get('car_id')
            ...
            if offers_deleted_count > 0 and car_id_to_delete:
                ...
                other_offers_count = other_offers_count_result[0] if isinstance(other_offers_count_result,
                                                                                tuple) else other_offers_count_result.get(
                    'count', 0)

                if other_offers_count == 0:
                    logger.info(
                        f"Немає інших пропозицій для car_id {car_id_to_delete}. Видалення авто та пов'язаних даних.")
                    # Викликаємо метод з CarRepository для видалення авто та його залежностей
                    CarRepository.delete_car_and_dependencies(car_id_to_delete, cursor)
                else:
                    logger.info(
    ...
    def get_filtered(
            ...
            fuel_type_key: Optional[str] = None,  # Назва параметра залишається fuel_type_key для внутрішньої логіки SQL
        ...
        base_sql = """
                   SELECT o.id             as offer_id,
                          ...
                          epd.battery_capacity_kwh,
                          ft.key_name      as fuel_type,
                          ft.label         as fuel_type_label, -- Аліас fuel_type для виведення
                          o.created_at     as offer_created_at,
                          cust.customs_payments_total_uah,
                          cust.final_total as final_price_uah
                   FROM offers o
        ...
        if make_name:
            where_clauses.append("cma.name = %s")
            params.append(make_name)
        if model_name:
            where_clauses.append("cm.name = %s")
        ...
        elif sort_by == 'oldest':  # Змінено сортування з offer_created_at на created_at (якщо це поле в offers)
            base_sql += " ORDER BY o.created_at ASC, o.id ASC"  # Або o.offer_created_at
        else:
            base_sql += " ORDER BY o.created_at DESC, o.id DESC"
